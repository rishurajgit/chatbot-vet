...
async def get_pet_visits(pet_id: int):
    ...
    Use this tool when the user asks for a pet's visit history.
    """

    return await vet_api.request(
        method="GET",
        endpoint=f"/pets/{pet_id}/visits",
    )
...
async def update_visit(
    ...
    Use this tool only when the visit ID is known.
    """
